...
public class AreaDBService {
	...
	public void batchInsert(List<DictData> list) {
		...
		for (DictData dictData : list) {
			...
			db.insert("area_dict", null, values);
		}

		db.setTransactionSuccessful();
		db.endTransaction();
		long b = System.currentTimeMillis();
		Log.e("AreaDBService-batchInsert-耗时->", String.valueOf(b - a));
	}
	...
	public <T> List<T> query(Class<T> ot, String upid) {
		...
		Cursor cs = db.query("area_dict", new String[] { "ID,UP_ID ,DICT_NAME,DICT_LEVEL" }, "UP_ID=?",
	...
	public void detele() {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		db.delete("area_dict", null, null);
	}

	public <T> List<T> queryViaLevel(Class<T> ot, String level) {
		List<T> list = new ArrayList<T>();
		SQLiteDatabase db = dbHelper.getReadableDatabase();
		Cursor cs = db.query("area_dict", new String[] { "ID,UP_ID ,DICT_NAME,DICT_LEVEL" }, "DICT_LEVEL=?",
